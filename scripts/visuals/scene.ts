...
import {cableAppearance} from '../../frontend/src/visual-assets/cableAppearance';
...
renderer.setSize(1200,760);renderer.setPixelRatio(1);renderer.setClearColor('#f4f7fb',1);
...
function specimen(mode='cutaway') {
 ...
 const solid=buildCableGeometry(cable,mode as 'cutaway',visible);solid.children[0].visible=false;group.add(solid,cableAppearance(cable,mode,visible));
...
if(kind==='cable') {
 ...
 camera.position.set(.4,.21,.48);camera.lookAt(-.014,0,0);camera.zoom=1.68;
