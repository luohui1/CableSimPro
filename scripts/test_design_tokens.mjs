...
assert.ok(parseFloat(t['text-control']) >= 14, 'desktop input typography must not shrink below 14px');
assert.ok(parseFloat(t['text-control-touch']) >= 16, 'touch input typography');
assert.ok(parseFloat(t['control-touch-height']) >= 44, 'touch input height');
