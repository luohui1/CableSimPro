...
`b28d22f` 的专项 design artifact `10087436346` 已核对：18 个浏览器用例全部通过，后端 243 项通过。但实际截图另发现项目卡片被导航 flex 容器压缩、以及窄屏敷设图仍使用过密的固定 0.1m 刻度。

后续修整让项目卡片不参与压缩，文字留在底面内；敷设画布改用以屏幕间距为约束的 1/2/5 米制主刻度（至少56px）与次网格（至少14px）。这只改变绘制密度，不改变坐标映射、输入单位、拖动约束或10mm吸附。窄屏有足够画布高度，回路标签缩为不拦截点击的小角标。增加七种比例的刻度检查，并在原有浏览器用例中加入边界/间距断言，不减少测试。
