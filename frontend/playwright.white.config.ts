...
export default defineConfig({...common,projects:common.projects?.filter(p=>p.name==='chromium'||p.name==='webkit').map(p=>({...p,testMatch:['white-workbench.spec.ts','industrial-workbench.spec.ts','progressive-workbench.spec.ts','light-studio.spec.ts','dual-mode.spec.ts','engineering-pulse.spec.ts']})),outputDir:'test-results-white',reporter:[['list'],['junit',{outputFile:'test-results-white/white-tests.xml'}],['html',{open:'never',outputFolder:'playwright-report-white'}]]});
