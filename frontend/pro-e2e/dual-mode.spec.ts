...
async function planTask(page:any){
 const planned=page.waitForResponse((r:any)=>r.request().method()==='POST'&&r.url().endsWith('/invoke')&&r.request().postDataJSON()?.capability==='task.plan');
 await page.getByRole('button',{name:'生成任务计划',exact:true}).click();
 const response=await planned;expect(response.ok()).toBe(true);
}
async function switchMode(page:any,name:'专业工作台'|'智能工程流',testId:'professional-mode'|'agent-mode'){
...
test('calculation and model survive mode switch with one workspace',async({page},info)=>{
 ...
 await switchMode(page,'智能工程流','agent-mode');await expect(page.locator('.flow-result-metrics')).toContainText(value.replace(/\s+/g,' ').trim());
 ...
 await switchMode(page,'专业工作台','professional-mode');await expect(canvas).toHaveAttribute('data-test-identity','preserved');
...
test('agent plan review applies real parameter change and restores after reload',async({page},info)=>{
 test.slow();
 await enter(page,'agent');await page.getByLabel('描述本次工程任务',{exact:true}).fill('截面积改为 400 mm²，重新计算');await planTask(page);
 ...
 await page.reload({waitUntil:'domcontentloaded'});await expect(proposal).toContainText('400');
 ...
 await expect(page.getByTestId('session-revision')).toHaveText('rev.2');await expect(page.locator('.flow-result-metrics')).toBeVisible();
 await switchMode(page,'专业工作台','professional-mode');await expect(page.locator('.enterprise-inspector').getByLabel('导体截面积',{exact:true})).toHaveValue('400');
 await expect(page).toHaveURL(/mode=workbench/);await page.reload({waitUntil:'domcontentloaded'});await expect(page.getByTestId('professional-mode')).toBeVisible();await expect(page.getByTestId('session-revision')).toHaveText('rev.2');await expect(page.locator('.enterprise-result-summary')).toBeVisible();
});
...
test('mode and browser history preserve task draft without changing revision',async({page})=>{
 ...
 await switchMode(page,'专业工作台','professional-mode');await page.goBack();await expect(page.getByTestId('agent-mode')).toBeVisible();await expect(input).toHaveValue('待补充的客户设计任务');
...
test('manual edit makes old agent proposal stale instead of overwriting engineering state',async({page})=>{
 test.slow();
 await enter(page,'agent');await page.getByLabel('描述本次工程任务',{exact:true}).fill('截面积改为 400 mm²，重新计算');await planTask(page);await expect(page.getByRole('region',{name:'待审查工程变更'})).toBeVisible();
 await switchMode(page,'专业工作台','professional-mode');const field=page.locator('.enterprise-inspector').getByLabel('导体截面积',{exact:true});await field.fill('300');await field.press('Tab');await expect(page.getByTestId('session-revision')).toHaveText('rev.2');
 await switchMode(page,'智能工程流','agent-mode');await expect(page.getByRole('button',{name:'批准并执行',exact:true})).toBeDisabled();await expect(page.locator('.flow-review')).toContainText('不可批准旧提案');
 ...
 await page.reload({waitUntil:'domcontentloaded'});await expect(page.getByTestId('session-revision')).toHaveText('rev.2');
 await expect(page.getByRole('region',{name:'待审查工程变更'})).toHaveCount(0);
});
...
test('uncommitted property survives mode switch and blocks plan and approval',async({page})=>{
 ...
 await switchMode(page,'智能工程流','agent-mode');await page.getByLabel('描述本次工程任务',{exact:true}).fill('计算载流量');await expect(page.getByRole('button',{name:'生成任务计划',exact:true})).toBeDisabled();
 await expect(page.locator('.dual-draft-warning')).toContainText('1 项未提交输入');await switchMode(page,'专业工作台','professional-mode');await expect(field).toHaveValue('');
...
test('explicit workspace link opens exact project in either mode',async({page,request})=>{
 ...
 await switchMode(page,'专业工作台','professional-mode');await expect(page.locator('.enterprise-inspector').getByLabel('导体截面积',{exact:true})).toHaveValue('300');
...
test('Chinese composition and shift enter do not send, hidden workbench cannot steal focus',async({page})=>{
 test.slow();
 await enter(page);await switchMode(page,'智能工程流','agent-mode');let plans=0;page.on('request',r=>{if(r.url().endsWith('/invoke')&&r.postDataJSON()?.capability==='task.plan')plans++});
 ...
 await planTask(page);await expect(page.getByRole('region',{name:'待审查工程变更'})).toBeVisible();expect(plans).toBe(1);
});
...
test('inspection output remains a current tool result without fabricated numeric metrics',async({page})=>{
 await enter(page,'agent');await page.getByLabel('描述本次工程任务',{exact:true}).fill('解释当前模型的假设');await planTask(page);
 const approved=page.waitForResponse(r=>r.request().method()==='POST'&&r.url().endsWith('/approve'));
 await page.getByRole('button',{name:'批准并执行',exact:true}).click();expect((await approved).ok()).toBe(true);await expect(page.getByTestId('session-revision')).toHaveText('rev.2');
 ...
 await page.reload({waitUntil:'domcontentloaded'});await expect(page.getByRole('region',{name:'任务结果'})).toContainText('不是完整 IEC 60287');await expect(page.locator('.flow-status')).toHaveText('工具已完成');
});
