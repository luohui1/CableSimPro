...
test('light entry renders two engineering visuals without creating a project or calling outside services',async({page},info)=>{
 ...
 await page.goto('/');await expect(page.getByRole('heading',{name:'选择你的工作方式'})).toBeVisible();await expect(page.getByTestId('engineering-illustration')).toHaveCount(2);
 await expect(page.getByTestId('engineering-illustration').first()).toHaveAttribute('data-status',/ready|fallback/);
