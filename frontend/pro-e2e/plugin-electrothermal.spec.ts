...
async function prepare(request:APIRequestContext,r20:boolean){
 ...
 if(r20){const edit=await request.post(`/api/workspaces/${w.id}/edit`,{data:{expected_revision:1,changes:[{path:'cable.r20_ohm_km',value:.0754}]}});expect(edit.ok()).toBe(true);w=await edit.json()}
 ...
 expect((await request.post('/api/plugins/install',{data:{plugin_id:pid,version:'0.1.0',plan_sha256:p.plan_sha256,state_revision:p.state_revision,approved:true,license_acknowledged:true,grants:Object.fromEntries(p.plugins.map((i:any)=>[i.plugin_id,i.permissions]))}})).ok()).toBe(true);
 const l=await (await request.get(`/api/plugins/workspaces/${w.id}/lock`)).json();
 expect((await request.post(`/api/plugins/workspaces/${w.id}/enable`,{data:{plugin_id:pid,expected_revision:w.revision,lock_revision:l.lock.lock_revision,enabled:true,approved:true}})).ok()).toBe(true);
 return w;
}
...
test('real ampacity bracket and domain comparison render without promoting the result',async({page,request},info)=>{
 ...
 expect(out.status).toBe('succeeded');expect(s.ampacity_a).toBeGreaterThan(0);expect(s.bracket.lower_max_temperature_c).toBeLessThanOrEqual(90);expect(s.bracket.upper_max_temperature_c).toBeGreaterThan(90);
