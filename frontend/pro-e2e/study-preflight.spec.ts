...
test('study preflight reads committed inputs, exports matching evidence and never writes',async({page},info)=>{
 await enter(page);let writes=0;page.on('request',r=>{if(!['GET','HEAD','OPTIONS'].includes(r.method()))writes++});
...
test('native target cannot pretend a COMSOL build or solve succeeded',async({page})=>{
 ...
 await expect(dialog).toContainText('不能声明兼容或模型已构建');await expect(dialog.getByRole('button',{name:/运行仿真|批准执行/})).toHaveCount(0);
});
