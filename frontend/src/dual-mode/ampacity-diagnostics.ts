...
export interface AmpacityDiagnostics {
 ...
 currentMarginA:number;temperatureMarginC:number;utilizationPercent:number;
 limitingPhase:string;phaseIndex:number;circuitLossKw:number;residualK:number;
...
export function buildAmpacityDiagnostics(result:Result):AmpacityDiagnostics{
 ...
 const summaryMargin=result.summary.thermal_margin_c;
 const temperatureMargin=summaryMargin??result.input.cable.max_temperature_c-conductorTemperature;
 const overloaded=currentMargin<0||result.summary.utilization_percent>100||temperatureMargin<0;
 ...
 const soilSelf=soilRow[phaseIndex]??0;
 const soilMutual=soilRow.reduce((sum,value,index)=>sum+(index===phaseIndex?0:value),0);
 return {
  ...
  limitingPhase:result.summary.limiting_phase,phaseIndex,circuitLossKw:state.circuit_loss_kw,residualK:state.residual_k,
  losses,dominantLoss,
  ...
  radialResistances,radialResistanceTotal:radialTotal,soilSelfKMW:soilSelf,soilMutualKMW:soilMutual,
