...
export default function AgentWorkspace({active,draft,setDraft,openWorkbench}:{active:boolean;draft:string;setDraft:(value:string)=>void;openWorkbench:(view?:string)=>void}){
 ...
 const [section,setSection]=useState<'task'|'selection'|'documents'>('task'),[provider,setProvider]=useState<'local'|'openai'>('local'),[consent,setConsent]=useState(false),[resultsOpen,setResultsOpen]=useState(true),[now,setNow]=useState(Date.now()),[editTask,setEditTask]=useState(false);
 ...
 const validOutput=validResult||s.outputCurrent;
 const compact=(pending||validOutput)&&!editTask;
 useEffect(()=>{if(p?.ready){setSection('task');setEditTask(false)}},[p]);
 useEffect(()=>{if(!active)return;const timer=window.setInterval(()=>setNow(Date.now()),1000);return()=>clearInterval(timer)},[active]);
 useEffect(()=>{if(!active)return;const key=(e:KeyboardEvent)=>{if((e.ctrlKey||e.metaKey)&&e.key.toLowerCase()==='k'){e.preventDefault();setSection('task');setEditTask(true);requestAnimationFrame(()=>input.current?.focus())}};window.addEventListener('keydown',key);return()=>window.removeEventListener('keydown',key)},[active]);
 ...
 return <main className="agent-workspace" aria-label="智能工程流工作区">
  ...
  <div hidden={section!=='task'} className="flow-task-layout">
   <section className="flow-task-main">
    <section className={`flow-composer ${compact?'is-compact':''}`} aria-label="工程任务输入"><header className="flow-composer-heading"><label htmlFor="flow-objective">描述本次工程任务</label>{(pending||validOutput)&&<button aria-expanded={!compact} onClick={()=>setEditTask(v=>!v)}>{compact?(pending?'查看任务输入':'发起新任务'):'收起输入'}</button>}</header>{compact&&<p className="flow-submitted-text">{latestRequirement||'当前工程结果已同步，可继续检查或发起新的研究任务。'}</p>}<div hidden={compact}><textarea id="flow-objective" ref={input} value={draft} maxLength={3000} placeholder="例如：将导体截面积改为 400 mm²，重新计算载流量。" onChange={e=>setDraft(e.target.value)} onCompositionStart={()=>{composing.current=true}} onCompositionEnd={()=>{composing.current=false}} onKeyDown={e=>{if(e.key==='Enter'&&!e.shiftKey&&!composing.current&&!e.nativeEvent.isComposing&&e.nativeEvent.keyCode!==229){e.preventDefault();send()}}}/><div className="flow-compose-actions"><label>任务解析<select aria-label="工程流解析方式" value={provider} onChange={e=>{setProvider(e.target.value as typeof provider);setConsent(false)}}><option value="local">本地明确命令 · 非大模型</option><option value="openai" disabled={!s.status.cloud_configured}>云端模型{s.status.cloud_configured?'':' · 未配置'}</option></select></label><button disabled={blocked||pending||!draft.trim()||(provider==='openai'&&(!consent||!s.status.cloud_configured))} onClick={send}><Send size={16}/>生成任务计划</button></div>
     ...
     {!pending&&<div className="flow-examples">{['计算载流量','截面积改为 400 mm²，重新计算','比较土壤热阻率 0.8、1.2、1.6 下的载流量'].map(t=><button key={t} onClick={()=>{setDraft(t);input.current?.focus()}}>{t}<ArrowRight size={13}/></button>)}</div>}</div>
    </section>
    {(latestRequirement||s.output)&&<div className="flow-stage-strip" aria-label="任务阶段"><span className={latestRequirement?'complete':''}>01 任务要求</span><span className={pending?'current':s.output?'complete':''}>02 人工审查</span><span className={s.busy?'current':validOutput?'complete':''}>03 工具执行</span><span className={validOutput?'complete':''}>04 查看结果</span></div>}
    {latestRequirement&&!compact&&<article className="flow-requirement"><span>本次要求</span><p>{latestRequirement}</p></article>}
    ...
    {s.output&&<section className="flow-artifact flow-output" aria-label="任务结果"><header><h2><CheckCircle2 size={19}/>计算与工具记录</h2><span>{validOutput?'与当前工况一致':'历史输出／非当前计算结论'}</span></header>{validOutput?<p>{s.output.statement}</p>:<p className="flow-warning">输入或版本已变化，以下记录只作历史依据；请重新计算当前工况。</p>}
