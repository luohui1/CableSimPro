...
import '../professional/pc-studio.css';
...
function Application(){
 ...
 return <div className={`dual-app light-studio pc-studio ${route.mode==='workbench'?'white-workbench progressive-studio reference-studio':''} ${route.mode?'is-project':'is-home'}`}>
  ...
  {route.mode==='agent'&&usable&&<details className="pc-agent-health"><summary>工程输入与方法状态 · {dirty?'有未提交输入':'查看输入、工具与依据'}</summary><EngineeringPulse mode={route.mode} openWorkbench={openWorkbench} openAgent={openAgent}/></details>}
  {route.mode==='agent'&&usable&&s.proposal?.ready&&<EngineeringReadinessPanel openWorkbench={openWorkbench}/>}
  ...
  {usable&&<>
   <div hidden={route.mode!=='workbench'} className="dual-mode-surface" data-testid="professional-mode">{visited.workbench&&<EnterpriseWorkbench embedded active={route.mode==='workbench'} onAgent={openAgent} requestedView={viewRequest} onHome={home} statusContent={<><EngineeringPulse mode="workbench" openWorkbench={openWorkbench} openAgent={openAgent}/>{s.proposal?.ready&&<EngineeringReadinessPanel openWorkbench={openWorkbench}/>}</>}/>}</div>
