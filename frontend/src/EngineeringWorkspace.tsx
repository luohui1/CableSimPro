...
import {AgentPanel,Inspector,ResultsPanel} from './StudioPanels';
...
function Workspace(){
 ...
 const canRun=['installation','results'].includes(active)&&domain!=='vertical_air';
 ...
 useEffect(()=>{const handler=(e:KeyboardEvent)=>{const el=e.target as HTMLElement;if(['INPUT','SELECT','TEXTAREA'].includes(el.tagName)||el.isContentEditable)return;
 ...
 return <div className={`eng-app ${assistant?'assistant-visible':''}`}>
 ...
 <div className="eng-body"><main className={`eng-main ${showProperties?'has-properties':''}`} id="engineering-content"><div className="eng-page-content">
 ...
 </div>{showProperties&&<aside className="eng-properties" aria-label="工程属性"><header><h2>参数</h2><span>校验后自动保存</span></header><div className="property-sections">{(active==='model'?[['cable','结构尺寸'],['materials','材料参数'],['study','运行条件']]:[['installation','敷设条件'],['study','运行条件']]).map(([id,label])=><button key={id} className={section===id?'active':''} onClick={()=>{setSection(id);s.select(id)}}>{label}</button>)}</div><Inspector/></aside>}</main>
 {assistant&&<aside className="eng-assistant" aria-label="工程助手面板"><header><span><MessageSquare size={16}/>工程助手</span><button aria-label="关闭工程助手" onClick={()=>setAssistant(false)}><X size={17}/></button></header><AgentPanel/></aside>}
 </div><footer className="eng-footbar"><span>单位：mm / m / A / °C</span><span>计算方法与适用范围见“设计依据”</span><button onClick={s.exportJSON}>导出工程</button><button onClick={()=>input.current?.click()} disabled={busy}>导入工程</button><button onClick={()=>void s.create()} disabled={busy}>新建工程</button></footer>
 </div>
...
export default function EngineeringWorkspace(){return <StudioProvider><Workspace/></StudioProvider>}
