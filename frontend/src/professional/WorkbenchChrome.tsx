...
export function WorkbenchBanner(){return <div className="wb-brand-scene" aria-label="工作台品牌背景"><div><b>连接工程 · 让设计更清晰</b><span>FROM STRUCTURE TO EVIDENCE</span></div><img src="/engineering/white-workbench/mountain-detail.webp" alt="" aria-hidden="true" width="348" height="134"/></div>}
...
export function WorkbenchAnalysis(){
 ...
 return <div className="wb-analysis-grid" aria-label="运行结果分析"><Panel title={`径向温度 · ${'ABC'[phase]} 相`} className="wb-radial-panel">{profile?<EngineeringChart label="当前运行径向温度曲线" xLabel="半径 / mm" yLabel="温度 / °C" height={200} series={[{name:'运行温度',points:profile.points.map(p=>[p.radius_mm,p.temperature_c])}]} threshold={r.input.cable.max_temperature_c}/>:<p className="wb-unavailable">当前运行工况无有效温度解，未绘制示例曲线。</p>}</Panel><Panel title="关键结果数据"><table className="wb-result-table"><thead><tr><th>项目</th><th>数值</th><th>单位</th></tr></thead><tbody>{rows.map(([name,value,unit])=><tr key={name}><th scope="row">{name}</th><td>{value}</td><td>{unit}</td></tr>)}</tbody></table><p className="wb-trace-note">{r.model_version}<br/>输入摘要 {r.input_sha256.slice(0,12)}… · rev.{s.w!.revision}</p></Panel></div>;
}
