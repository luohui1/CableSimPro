...
export default function EngineeringCanvas(){
 ...
 <Text x={width-232} y={height-60} text={`均匀土壤\nρth = ${fmt(e.soil_rho_k_m_w,2)} K·m/W`} lineHeight={1.7} fontSize={12} fill="#435b4d"/>
 <Rect width={32} height={height} fill="#eef2f5"/><Rect width={width} height={28} fill="#eef2f5"/>
 {Array.from({length:41},(_,i)=>(i-20)/10).map((v,i)=><Group key={i}>{X(v)>40&&X(v)<width&&<><Line points={[X(v),20,X(v),28]} stroke="#8798a9"/><Text x={X(v)-16} y={5} width={32} align="center" text={v.toFixed(1)} fontSize={9} fill="#2f4b65"/></>}{Y(v)>45&&Y(v)<height-20&&<><Line points={[23,Y(v),32,Y(v)]} stroke="#8798a9"/><Text x={2} y={Y(v)-5} text={v.toFixed(1)} fontSize={9} fill="#2f4b65"/></>}</Group>)}
 ...
 <Text x={48} y={height-22} text={`1 回路 / 3 × 单芯 · ${e.arrangement==='flat'?'水平排列':'等边三角排列'} · 尺度一致`} fontSize={10} fill="#39556c"/>
 </Layer></Stage>
