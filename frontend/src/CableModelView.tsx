...
import {cableAppearance,disposeScene} from './engineering-visuals/cableAppearance';
...
export default function CableModelView({cable}:{cable:Cable}){
 ...
 const fit=useRef<()=>void>(()=>{}),userMoved=useRef(false);
 ...
 useEffect(()=>{
  ...
  const resize=()=>{const w=el.clientWidth,h=el.clientHeight;if(w<2||h<2)return;const aspect=w/h;camera.left=-.25*aspect;camera.right=.25*aspect;camera.top=.25;camera.bottom=-.25;camera.updateProjectionMatrix();if(!userMoved.current)fit.current();renderer.setSize(w,h);render()};
  const ro=new ResizeObserver(resize);ro.observe(el);ctl.addEventListener('change',render);ctl.addEventListener('start',()=>{userMoved.current=true});resize();setReady(true);
 ...
 useEffect(()=>{const c=cam.current,ctl=controls.current;if(!c||!ctl)return;
  ...
  fit.current();return()=>{userMoved.current=false;
  fit.current=()=>{}};
