...
export default function PluginCenter(){
 const [catalog,setCatalog]=useState<Catalog|null>(null),[projects,setProjects]=useState<Project[]>([]),[project,setProject]=useState('');
 ...
 const enabled=!!lock?.lock.plugins.some(p=>p.plugin_id===selected&&p.release_sha256===item?.release_sha256);
 ...
 const numbersValid=[heat,surface,metalK,conductorK].every(x=>x.trim()!==''&&Number.isFinite(Number(x)));
 ...
 async function execute(){if(!m||!command||!lock)return;const arguments_:Record<string,unknown>={};
  ...
  const r=await request(`/api/plugins/workspaces/${project}/invoke`,{plugin_id:m.plugin_id,command:command.id,request_id:crypto.randomUUID(),expected_revision:lock.lock.project_revision,lock_sha256:lock.lock_sha256,arguments:arguments_,confirmed:true});setResult(r);await refresh();
 }
 return <main className="plugin-center">
  <header className="plugin-top"><a href="/" aria-label="返回工程首页"><ArrowLeft size={17}/>CableSimPro</a><span>插件中心 <small>PREVIEW</small></span><button disabled={busy} onClick={()=>void act(refresh)}>刷新目录</button></header>
  ...
  <section className="plugin-discovery" aria-label="插件目录"><div className="plugin-heading"><div><h1>把工程能力接入项目</h1><p>按需安装，按项目启用。能力范围与验证证据分开查看。</p></div><span className="plugin-local">本机目录</span></div>
   ...
   <div className="plugin-project"><label htmlFor="plugin-project">当前项目</label><select id="plugin-project" value={project} disabled={busy} onChange={e=>setProject(e.target.value)}><option value="">只浏览目录，不创建工程</option>{projects.map(p=><option value={p.id} key={p.id}>{p.name}</option>)}</select>{lock&&<button onClick={()=>save('project-plugin-lock.json',lock)}><Download size={15}/>导出插件锁</button>}</div>
  ...
  {item&&m&&<aside className="plugin-detail" aria-label="插件详情"><div className="plugin-detail-heading"><span>{labels[m.category]}</span><h2>{m.name}</h2><p>{m.publisher}</p><small>{m.version} · API 1</small></div>
   ...
   {m.distribution==='bundled-adapter'&&<details className="plugin-run"><summary>适配器验证台</summary><p>读取当前项目已保存快照。生成独立工件，不覆盖当前载流量结果。CAD 默认展示段长 0.25 m；网格为本体六层。</p>
    ...
    <button className="plugin-primary" disabled={busy||!enabled||!item.runtime.metadata_ready||!project||(needsSource&&!source)||(thermal&&!numbersValid)} onClick={()=>void act(execute)}>{busy?'正在执行…':`执行：${command?.title}`}</button>
