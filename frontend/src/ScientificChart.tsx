...
export function EngineeringChart({series,xLabel,yLabel,label,threshold}:{series:Series[];xLabel:string;yLabel:string;label:string;threshold?:number}){
 ...
 useEffect(()=>{
  ...
  c.setOption({animation:false,color:['#256a57','#b56635','#315783','#646464'],textStyle:{fontFamily:'IBM Plex Sans, Noto Sans CJK SC, sans-serif',color:'#3a3a3a'},
   ...
   aria:{enabled:true,label:{description:label}},xAxis:{type:'value',name:xLabel,nameLocation:'middle',nameGap:34,splitLine:{show:false}},
   yAxis:{type:'value',name:yLabel,nameGap:22,scale:true,splitLine:{lineStyle:{color:'#e8e8e6'}}},
 ...
 },[series,xLabel,yLabel,label,threshold]);
 const csv=()=>download(label+'.csv','series,x,y\n'+series.flatMap(s=>s.points.map(p=>`"${s.name.replaceAll('"','""')}",${p[0]},${p[1]}`)).join('\n'),'text/csv;charset=utf-8');
 return <div className="scientific-chart"><div className="chart-actions"><span>{label}</span><button onClick={()=>chart.current?.dispatchAction({type:'dataZoom',start:0,end:100})}>恢复范围</button><button onClick={csv}>导出数据</button></div><div ref={host} role="img" aria-label={label} style={{width:'100%',height:340}}/>
 <details className="chart-data"><summary>查看曲线数据表</summary><table><thead><tr><th>曲线</th><th>{xLabel}</th><th>{yLabel}</th></tr></thead><tbody>{series.flatMap(s=>s.points.map((p,i)=><tr key={s.name+i}><td>{s.name}</td><td>{p[0].toPrecision(6)}</td><td>{p[1].toPrecision(6)}</td></tr>))}</tbody></table></details></div>;
}
