...
export function EngineeringChart({series,xLabel,yLabel,label,threshold,height=340}:{series:Series[];xLabel:string;yLabel:string;label:string;threshold?:number;height?:number}){
 ...
 useEffect(()=>{
  ...
  c.setOption({animation:false,color:['#256a57','#b56635','#315783','#646464'],textStyle:{fontFamily:'IBM Plex Sans, Noto Sans CJK SC, sans-serif',color:'#3a3a3a'},
   grid:{left:compact?54:70,right:compact?24:32,top:compact?44:68,bottom:compact?54:88},tooltip:{trigger:'axis',confine:true},legend:{top:compact?0:3},
   aria:{enabled:true,label:{description:label}},xAxis:{type:'value',name:displayXLabel,nameLocation:'middle',nameGap:compact?25:34,splitLine:{show:false}},
   yAxis:{type:'value',name:displayYLabel,nameGap:compact?14:22,scale:true,splitLine:{lineStyle:{color:'#e8e8e6'}}},
   dataZoom:[{type:'inside',filterMode:'none'},{type:'slider',height:compact?10:14,bottom:compact?2:8,showDetail:false}],
