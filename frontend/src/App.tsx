...
export default function App() {
  ...
  const validSweep = sweep?.signature === signature ? sweep.data : null;
  ...
  return <div className={`studio ${landing ? 'at-home' : ''} ${agentVisible ? '' : 'agent-hidden'}`}>
    <aside className="model-sidebar">
      ...
      <nav className="model-tree" aria-label="模型树"><button className={mode === 'model' && !landing ? 'selected root-node' : 'root-node'} onClick={() => {setLanding(false); setMode('model');}}><Icon name="folder" size={16}/><span>{scenario.name}</span></button>
        <p>模型定义</p>{TREE.slice(0,3).map(item => <button key={item.id} className={node === item.id && mode === item.mode && !landing ? 'selected' : ''} onClick={() => selectNode(item)}><Icon name={item.icon} size={15}/>{item.label}</button>)}
        <p>研究</p>{TREE.slice(3,5).map(item => <button key={item.id} className={node === item.id && mode === item.mode && !landing ? 'selected' : ''} onClick={() => selectNode(item)}><Icon name={item.icon} size={15}/>{item.label}<small>{item.id === 'study' ? '1' : ''}</small></button>)}
        <p>结果</p>{TREE.slice(5).map(item => <button key={item.id} className={mode === item.mode && !landing ? 'selected' : ''} onClick={() => selectNode(item)}><Icon name={item.icon} size={15}/>{item.label}<small>{item.id === 'results' ? current ? '✓' : '—' : snapshots.length || ''}</small></button>)}
      </nav>
    ...
    <main className="studio-main">
      ...
      {landing ? <section className="start-page"><div className="start-content"><p className="start-kicker">电缆建模与计算</p><h1>这次要研究什么？</h1><Composer {...composer}/><div className="reference-model"><Icon name="cube" size={18}/><div><strong>12/20 kV · Cu 240 mm²</strong><span>参考模型 · 三根单芯 XLPE · 均匀土壤直埋</span></div><button aria-label="检查参考参数" onClick={() => selectNode(TREE[0])}><Icon name="arrow"/></button></div><div className="suggestions">{SUGGESTIONS.map((s,i) => <button key={s} disabled={busy} onClick={() => send(s)}><span>{['计算参考方案','研究土壤热阻率的影响','检查模型假设'][i]}</span><Icon name="arrow" size={15}/></button>)}</div><p className="start-boundary">先审查模型，再执行求解。当前版本使用稳态热网络，尚未完成工程认证。</p></div></section> : <>
