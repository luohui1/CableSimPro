...
function Workbench(){
 ...
 function onReady(event:DockviewReadyEvent){
  ...
  let restored=false;
  try{const saved=localStorage.getItem('cablesim-layout-v04');if(saved){a.fromJSON(JSON.parse(saved));restored=['tree','inspector','viewport','agent','results'].every(id=>a.getPanel(id));if(!restored)a.clear()}}catch{a.clear()}
  if(!restored){
   a.addPanel({id:'viewport',component:'viewport',title:'模型视图'});
   a.addPanel({id:'tree',component:'tree',title:'工程浏览器',position:{referencePanel:'viewport',direction:'left'},initialWidth:268});
   a.addPanel({id:'inspector',component:'inspector',title:'属性编辑',position:{referencePanel:'tree',direction:'below'},initialHeight:390});
   a.addPanel({id:'agent',component:'agent',title:'工程 Copilot',position:{referencePanel:'viewport',direction:'right'},initialWidth:390});
  ...
  a.onDidLayoutChange(()=>{try{localStorage.setItem('cablesim-layout-v04',JSON.stringify(a.toJSON()))}catch{}});
 }
 async function openProjects(){try{setProjects(await api('/api/workspaces'));setDialogError('')}catch(e){setDialogError(errorText(e))}}
 function resetLayout(){localStorage.removeItem('cablesim-layout-v04');setLayoutKey(k=>k+1)}
 ...
 useEffect(()=>{const key=(e:KeyboardEvent)=>{const t=e.target as HTMLElement;if(['INPUT','TEXTAREA','SELECT'].includes(t.tagName)||t.isContentEditable)return;if((e.ctrlKey||e.metaKey)&&e.key.toLowerCase()==='z'){e.preventDefault();if(!busy)void history(e.shiftKey?'redo':'undo')}if(e.key==='F9'){e.preventDefault();if(!busy&&!hub)void run()}};window.addEventListener('keydown',key);return()=>window.removeEventListener('keydown',key)},[history,run,busy,hub]);
 ...
 <main className="dock-host">{hub&&<DesignHub close={()=>setHub(false)}/>}<div style={{height:'100%',display:hub?'none':'block'}}>{mobile?<div className="mobile-workbench"><div className="mobile-tabs">{[['model','模型'],['properties','属性'],['agent','Copilot'],['results','结果']].map(([id,label])=><button key={id} className={id===mobileTab?'active':''} onClick={()=>setMobileTab(id)}>{label}</button>)}</div><div className="mobile-content">{mobileTab==='model'?<Viewport/>:mobileTab==='properties'?<><div className="mobile-property-select">{[['installation','敷设'],['cable','电缆'],['materials','材料'],['study','运行']].map(([id,label])=><button key={id} onClick={()=>s.select(id)}>{label}</button>)}</div><Inspector/></>:mobileTab==='agent'?<AgentPanel/>:<ResultsPanel/>}</div></div>:<DockviewReact key={layoutKey} className="dockview-theme-light" theme={themeLight} components={components} onReady={onReady} disableFloatingGroups keyboardNavigation/>}</div></main>
