...
export function cableAppearance(cable:Cable,mode:string,visible:boolean[]){
 ...
 if(visible[0]){
  ...
  const material=new THREE.MeshStandardMaterial({color:cable.conductor==='copper'?'#c4864e':'#bdc8d2',metalness:.84,roughness:.29});
  for(let ring=0;ring<=3;ring++){
   ...
   for(let n=0;n<count;n++){matrix.makeRotationX(n/count*2*Math.PI);mesh.setMatrixAt(n,matrix)}
 ...
 if(visible[4]&&mode!=='assembled'){
  ...
  const points=Array.from({length:21},(_,j)=>{
   ...
   return new THREE.Vector3(x,(ro-relief*.35)*Math.cos(a),(ro-relief*.35)*Math.sin(a));
  });
  const geometry=new THREE.TubeGeometry(new THREE.CatmullRomCurve3(points),20,relief,6,false);
  const material=new THREE.MeshStandardMaterial({color:'#b87944',metalness:.87,roughness:.3});
  const mesh=new THREE.InstancedMesh(geometry,material,56),matrix=new THREE.Matrix4();
  for(let n=0;n<56;n++){matrix.makeRotationX(n/56*Math.PI*2);mesh.setMatrixAt(n,matrix)}
