...
/** View preferences are supplied by the shell. Legacy groups default to expanded. */
export const DisclosureSettings=createContext<{initialOpen:string[];focusGroup?:string;focusSerial?:number}|null>(null);
/** Keep fields mounted: collapsing never discards a draft or a parameter lock. */
export function DisclosureGroup({title,count,draftCount=0,lockedCount=0,children}:{
 ...
 const settings=useContext(DisclosureSettings);
 const [expanded,setExpanded]=useState(()=>settings?settings.initialOpen.includes(title):true);
 const id=useId();
 useEffect(()=>{if(settings?.focusGroup===title)setExpanded(true)},[settings?.focusGroup,settings?.focusSerial,title]);
