...
export function MetricTile({label,value,unit,detail,icon,indicator,tone='neutral'}:{label:string;value:string;unit?:string;detail:string;icon:ReactNode;indicator?:ReactNode;tone?:Tone}){
 return <div className={`wb-metric wb-tone-${tone}`}><span className="wb-metric-icon" aria-hidden="true">{icon}</span><div><span className="wb-metric-label">{label}</span><strong>{value}{' '}<small>{unit}</small></strong><span className="wb-metric-detail">{detail}</span>{indicator}</div></div>;
}
