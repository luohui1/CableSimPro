# CableSimPro 0.4 · 电缆设计与资料工作台

开发分支 `feat/cable-design-v04`，基于 v0.3，尚未合并 main。

**高对比工程界面 + OCR / Agent 接入 + 企业资料页校对与引用 + 型号库反向选型 + 专项场分析。**

> 本机工程辅助 Preview，不是完整 IEC 60287、通用多物理场或已经认证的设计软件。当前载流量内核仍是单回路三根相同无铠装单芯电缆、均匀土壤直埋、平衡稳态模型。物理竖井/垂直敷设尚未建立通风与轴向热模型，不可套用直埋计算。
...
```sh
git clone --branch feat/cable-design-v04 https://github.com/luohui1/CableSimPro.git
cd CableSimPro
python scripts/run_demo.py
```
...
2. **型号库**：检查当前电缆的全部参数，填型号/企业/可选价格和来源，确认保存。仅提取了 4 类字段不意味着整根电缆已由厂家资料定义。
3. **反向选型**：输入目标电流与预留；默认只用用户核对型号。没有厂家数据时可明确勾选演示型号。查看失败原因和可行候选，送审后才应用。
4. **场分析**：分别运行土壤数值热场、绝缘电场、外部磁场。129×129 热场会增加 65×65 的对照；检查方法假设、数值残差和输入版本。

资料提取目前限带明确标签/单位的截面积、R20、绝缘/护套厚度，多型号表需要选取单型号节选；不是无监督全型号表识别。资料问答的引用编号会校验，但不能保证每条模型解释正确。

## 数据与工程限制

SQLite 及企业原件默认在 `.data/` 内。资料、型号快照、审批、运行和选型研究持久化。完整备份请停服后备份整个数据目录。工程 JSON 仅包含计算参数，不包含完整资料、历史、锁和凭据。

本版未做：铠装、排管、多回路、土壤干燥、循环/应急、物理竖井气流、缺陷电场、屏蔽涡流、压降/短路/保护协调完整设计；也没有自动查价、向量资料检索或无限自主 Agent。价格缺失不能宣称成本最优；全部候选失败时报告无可行方案，不放宽边界。
